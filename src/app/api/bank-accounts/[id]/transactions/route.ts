...
export async function GET(_: NextRequest, context: RouteContext<"/api/bank-accounts/[id]/transactions">) {
  try {
    await connect();
    ...
    const transactions = await BankTransactionModel.find({ user: userId, bankAccount: id })
...
export async function POST(request: NextRequest, context: RouteContext<"/api/bank-accounts/[id]/transactions">) {
  try {
    await connect();
    const userId = await getUserId();
    const { id } = await context.params;
    ...
    const transaction = await BankTransactionModel.recordTransaction({
      user: userId,
      bankAccount: id,
      type,
      amount,
