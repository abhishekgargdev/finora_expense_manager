  ...
  try {
    ...
    const bankAccount =
      input.bankAccount === undefined
        ? expense.bankAccount?.toString()
        : await ensureBankAccount(userId, input.bankAccount);
    const creditCard =
      input.creditCard === undefined
        ? expense.creditCard?.toString()
        : await ensureCreditCard(userId, input.creditCard);
    if (paymentMode === "Credit Card" && !creditCard) throw new Error("Choose a credit card.");
    if (["UPI", "Debit Card", "Bank Transfer"].includes(paymentMode) && !bankAccount)
    ...
    return NextResponse.json({ error: message }, { status: message === "Expense entry not found." ? 404 : 400 });
  }
}

export async function PUT(request: NextRequest, context: RouteContext<"/api/expenses/[id]">) {
  return updateExpense(request, context);
}
export async function PATCH(request: NextRequest, context: RouteContext<"/api/expenses/[id]">) {
  return updateExpense(request, context);
}
