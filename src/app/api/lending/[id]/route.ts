...
async function updateLending(request: NextRequest, context: RouteContext<"/api/lending/[id]">) {
  try {
    ...
    Object.assign(lending, parseLending(await request.json(), true));
    if (lending.amountReturned > lending.amount) throw new Error("Returned amount cannot exceed the original amount.");
    await lending.save();
