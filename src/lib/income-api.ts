...
const modes = ["Cash", "Bank Transfer", "UPI", "Other"] as const;
type Input = {
  amount?: unknown;
  source?: unknown;
  category?: unknown;
...
export function parseIncome(input: Input, partial = false) {
  ...
  if (input.category !== undefined || !partial) values.category = text(input.category) || undefined;
...
export function serializeIncome(entry: {
  ...
  const bankAccount =
    entry.bankAccount && typeof entry.bankAccount === "object" && "_id" in entry.bankAccount
...
export async function ensureBankAccount(userId: string, bankAccount?: unknown) {
  const id = text(bankAccount);
  if (!id) return undefined;
  if (!(await BankAccountModel.findOne({ _id: id, user: userId }).select("_id").lean()))
