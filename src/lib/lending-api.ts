...
const dateValue = (value: unknown, label: string, required = true) => { const raw = text(value); if (!raw && !required) return null; const date = new Date(raw); if (!raw || Number.isNaN(date.getTime())) throw new Error(`A valid ${label} is required.`); return date; };
